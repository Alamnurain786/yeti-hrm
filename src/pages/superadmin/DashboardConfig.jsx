...
const DashboardConfig = () => {
  const { user } = useAuth();
  const { dashboardConfig, dashboardCards, setDashboardConfigForRole } =
    useMockData();
    ...
    return (
      <div className="p-8 bg-white rounded-2xl shadow-sm border border-slate-100">
        <p className="text-slate-600">
          Only superadmin can configure dashboard cards.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-800 flex items-center">
          <LayoutGrid className="mr-2 text-blue-600" /> Dashboard Card
          Configuration
        </h1>
        <p className="text-slate-500 mt-1">
          Assign which summary cards appear for each role.
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {rolesToConfigure.map((role) => (
          ...
          >
            <h2 className="text-lg font-semibold text-slate-800 capitalize mb-4">
              {role} View
            </h2>
